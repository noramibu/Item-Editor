...
public final class StorageCommands {
    ...
    public static void register() {
        ClientCommandRegistrationCallback.EVENT.register(
                (dispatcher, access) -> dispatcher.register(ClientCommandManager.literal("storage")
                        .executes(context -> openStorage(context, 1, "", StorageSortMode.REGULAR))
                        .then(ClientCommandManager.literal("pages").executes(StorageCommands::openStoragePages))
                        .then(ClientCommandManager.literal("page")
                                .then(ClientCommandManager.argument("pageNumber", IntegerArgumentType.integer())
                                        .executes(context -> openStorage(
                                                context,
                        ...
                        .then(ClientCommandManager.literal("s")
                                .then(ClientCommandManager.argument("query", StringArgumentType.greedyString())
                                        ...
                                        .executes(context -> openStorage(
                                                ...
                                                StringArgumentType.getString(context, "query"),
                                                StorageSortMode.REGULAR))))
                        .then(ClientCommandManager.literal("sort")
                                .then(ClientCommandManager.argument("mode", StringArgumentType.word())
                                        .suggests((context, builder) ->
                                                SharedSuggestionProvider.suggest(SORT_MODE_SUGGESTIONS, builder))
                                        .executes(context -> {
                                            ...
                                            if (sortMode == null) {
                                                context.getSource()
                                                        .sendError(ItemEditorText.tr(
                                                                        "storage.command.unknown_sort_mode", modeToken)
                                                                .copy()
    ...
    private static int openStorage(
            CommandContext<FabricClientCommandSource> context, int page, String query, StorageSortMode sortMode) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.player == null || minecraft.level == null) {
            context.getSource()
                    .sendError(
                            ItemEditorText.tr("storage.command.no_world").copy().withStyle(ChatFormatting.RED));
            return 0;
        }
    ...
    private static int openStoragePages(CommandContext<FabricClientCommandSource> context) {
        ...
        minecraft.execute(() -> minecraft.setScreen(new StoragePagesScreen(minecraft, null)));
        return 1;
    }

    private static CompletableFuture<Suggestions> suggestSearchQuery(
            CommandContext<FabricClientCommandSource> ignoredContext, SuggestionsBuilder builder) {
