...
public final class CombatEditorPanel implements EditorPanel {
    ...
    public List<EditorSearchDialog.Target> searchTargets() {
        return CombatSpecialDataSection.searchTargets(this.context);
    }

    @Override
    public UIComponent build() {
        return SectionedEditorPanel.build(List.of(
                SectionedEditorPanel.Section.always(() -> CombatSpecialDataSection.buildDamageResistant(this.context)),
                SectionedEditorPanel.Section.always(() -> CombatSpecialDataSection.buildBehavior(this.context)),
                SectionedEditorPanel.Section.always(() -> CombatSpecialDataSection.buildBlocksAttacks(this.context)),
                SectionedEditorPanel.Section.always(() -> CombatSpecialDataSection.buildEquipment(this.context))));
    }
}
