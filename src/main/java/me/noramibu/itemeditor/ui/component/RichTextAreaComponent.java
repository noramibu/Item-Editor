...
public final class RichTextAreaComponent extends TextAreaComponent implements GreedyInputUIComponent {
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        ...
        if (doubled) {
            this.moveCursorToPoint(click.x(), click.y(), false);
            this.editBox.selectWordAtCursor();
        } else {
            this.moveCursorToPoint(click.x(), click.y(), click.hasShiftDown());
        }
    ...
    public boolean mouseDragged(MouseButtonEvent click, double deltaX, double deltaY) {
        ...
        this.moveCursorToPoint(click.x(), click.y(), true);
        this.afterCursorMove();
        return true;
    }
    ...
    private void moveCursorToPoint(double mouseX, double mouseY, boolean selecting) {
        int position = this.positionAt(mouseX, mouseY);
        this.editBox.setSelecting(selecting);
        this.editBox.seekCursor(Whence.ABSOLUTE, position);
    }

    private int positionAt(double mouseX, double mouseY) {
        double contentX = mouseX - this.getInnerLeft();
        double contentY = mouseY - this.getInnerTop() + this.scrollAmount();
        int lineIndex = (int) Math.floor(Math.max(0, contentY) / LINE_HEIGHT);
        lineIndex = Math.max(0, Math.min(lineIndex, this.layoutLines.size() - 1));
        return this.layoutLines.get(lineIndex).positionForX(contentX);
    }
    ...
    private int innerContentWidth() {
        return Math.max(1, this.getWidth() - this.totalInnerPadding());
    }
