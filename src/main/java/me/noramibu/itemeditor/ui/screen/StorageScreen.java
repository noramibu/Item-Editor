...
public final class StorageScreen extends ContainerScreen {
    ...
    protected void renderLabels(@NotNull GuiGraphics context, int mouseX, int mouseY) {
        context.drawString(this.font, this.storageTitleLabel, this.titleLabelX, this.titleLabelY, -12566464, false);
        context.drawString(
    ...
    protected void slotClicked(@Nullable Slot slot, int slotId, int button, @NotNull ClickType input) {
        ...
        if (this.isLockMode() && this.isStorageSlotId(slotId)) {
            if (slot != null && slot.hasItem()) {
                if (button == 0 && input == ClickType.QUICK_MOVE) {
                    this.copyStorageItemToInventory(slot);
                } else if (input == ClickType.PICKUP && button == 0) {
                    this.copyStorageItem(slot);
                } else if (input == ClickType.PICKUP && button == 1) {
                    this.openStorageItem(slot.index);
                }
            }
            return;
        }
        if (this.isLockMode() && input != ClickType.PICKUP) {
            return;
        }
    ...
    private void renderPanelText(GuiGraphics context) {
        ...
        context.drawString(
                this.font, Component.literal(fittedHeader), this.leftPos + 8, this.topPos + 6, UiColors.SUCCESS);
