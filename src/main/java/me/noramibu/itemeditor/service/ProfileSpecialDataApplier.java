...
final class ProfileSpecialDataApplier extends AbstractPreviewApplierSupport implements SpecialDataApplier {
    ...
    public void apply(SpecialDataApplyContext context) {
        if (Objects.equals(context.special().profileName, context.baselineSpecial().profileName)
                ...
                && Objects.equals(
                        context.special().profileTextureSignature, context.baselineSpecial().profileTextureSignature)) {
        ...
        if (profileName.isBlank()
                && profileUuidRaw.isBlank()
                && textureInputRaw.isBlank()
                && textureSignature.isBlank()) {
        ...
        if (!textureInputRaw.isBlank()) {
            ...
            if (!HeadTextureUtil.isBase64(textureValue)) {
                context.messages()
                        .add(ValidationMessage.error(ItemEditorText.str("preview.validation.profile_texture")));
                textureValue = "";
            }
        }
        ...
        if (!textureValue.isBlank()) {
            ...
            context.previewStack()
                    .set(
                            ...
                            ResolvableProfile.createResolved(new GameProfile(
                                    profileUuid == null ? Util.NIL_UUID : profileUuid,
                                    profileName,
                                    new PropertyMap(mutableProperties))));
            return;
        }

        if (!textureSignature.isBlank()) {
            context.messages()
                    .add(ValidationMessage.warning(
                            ItemEditorText.str("preview.validation.profile_signature_without_texture")));
        }
