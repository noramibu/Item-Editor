...
final class EnchantmentsPreviewApplier extends AbstractPreviewApplierSupport implements ItemPreviewApplier {
    ...
    public void apply(ItemPreviewApplyContext context) {
        if (this.sameEnchantmentDrafts(context.state().enchantments, context.baselineState().enchantments)
                && this.sameEnchantmentDrafts(
                        context.state().storedEnchantments, context.baselineState().storedEnchantments)
            ...
            this.restoreOriginalComponent(
                    context.originalStack(), context.previewStack(), DataComponents.STORED_ENCHANTMENTS);
            return;
        }
    ...
    private ItemEnchantments buildEnchantments(
        ...
        for (ItemEditorState.EnchantmentDraft draft : drafts) {
            ...
            Integer level = ValidationUtil.parseInt(
                    draft.level, ItemEditorText.str("enchantments.entry.level"), 1, maxLevel, messages);
            ...
            if (!unsafeAllowed) {
                if (level < enchantment.value().getMinLevel()
                    ...
                    messages.add(ValidationMessage.error(ItemEditorText.str(
                            "preview.validation.enchantment_level",
                            draft.enchantmentId,
                ...
                if (!enchantment.value().canEnchant(preview)) {
                    messages.add(ValidationMessage.warning(
                            ItemEditorText.str("preview.validation.enchantment_applicable", draft.enchantmentId)));
                }
            }
    ...
    private boolean sameEnchantmentDrafts(
            List<ItemEditorState.EnchantmentDraft> current, List<ItemEditorState.EnchantmentDraft> baseline) {
        return this.sameList(
                current,
                baseline,
                (left, right) -> Objects.equals(left.enchantmentId, right.enchantmentId)
