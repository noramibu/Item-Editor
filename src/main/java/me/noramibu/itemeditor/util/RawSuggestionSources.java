...
final class RawSuggestionSources {
    ...
    static Key key(
        ...
        return new Key(
                ...
                RawAutocompleteHints.objectKeyHints(
                        context.containerKey(), context.containerPath(), itemId, activeProfiles, registryAccess),
                RawAutocompleteHints.componentFieldHints(
                        context.containerKey(), itemId, activeProfiles, registryAccess),
                RawAutocompleteHints.dynamicKeyHints(context.containerKey(), context.containerPath(), registryAccess));
    }
    ...
    static Value value(
        ...
        slotType = RawAutocompleteHints.refineSlotTypeWithSiblingValues(
                slotType, currentKey, context.containerPath(), siblingValues);
        ...
        if (slotType == RawSlotType.VALUE_UNKNOWN) {
            expectedModes =
                    RawAutocompleteHints.expectedValueModes(currentKey, context.containerPath(), prefix, insideQuote);
            expectedModes = RawAutocompleteHints.refineExpectedModesWithSiblingValues(
                    currentKey, context.containerPath(), siblingValues, expectedModes, insideQuote);
        ...
        List<String> catalogHints =
                RawAutocompleteHints.contextualValueHints(currentKey, context.containerPath(), registryAccess);
        ...
        List<String> registryHints = RawAutocompleteHints.registryHintsForSlot(
                slotType, currentKey, context.containerKey(), context.containerPath(), registryAccess);
        registryHints = RawAutocompleteHints.mergeUnique(
                RawAutocompleteHints.siblingRegistryValueHints(
                        currentKey, context.containerPath(), siblingValues, registryAccess),
                registryHints);
        ...
        List<String> typedCatalogHints = RawAutocompleteHints.filterValuesByModes(catalogHints, expectedModes);
        List<String> typedRegistryHints = RawAutocompleteHints.filterValuesByModes(registryHints, expectedModes);
        if (slotType == RawSlotType.VALUE_FLOAT) {
            ...
            typedCatalogHints = RawAutocompleteHints.filterNumericHintsForCurrentKey(
                    typedCatalogHints, currentKey, context.containerPath());
            typedRegistryHints = RawAutocompleteHints.filterNumericHintsForCurrentKey(
                    typedRegistryHints, currentKey, context.containerPath());
        }
    ...
    private static void addKeySource(
        ...
        if (nearest) {
            output.addNearestSuggestions(
                    values, prefix, RawAutocompleteUtil.SuggestionKind.KEY, keyInsert, rank, source, reason);
        } else {
